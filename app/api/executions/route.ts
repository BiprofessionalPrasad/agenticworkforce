...
// GET /api/executions?workflowId=xxx  (list, optionally filtered)
export async function GET(request: NextRequest) {
  try {
    const { searchParams } = request.nextUrl;
    const workflowId = searchParams.get("workflowId") || undefined;
    const executions = await getAllExecutions(workflowId);
...
// POST /api/executions - store an execution history record
export async function POST(request: NextRequest) {
  try {
    ...
    const saved = await saveExecution(execInput as any);
